...
public class GPSManager : MonoBehaviour
{
    ...
    float latitude = 37.513728f;
    float longtitude = 127.029534f;
    Vector2 myLocation;
    ...
    {
        logTxt.text += $"Angle({angle}), myLocation({latitude}, {longtitude}))\n";

        angle *= Mathf.Deg2Rad;

        // 내 GPS 좌표 기준, Pokemon 오브젝트의 GPS(x1, y1)를 계산
        float x1 = Mathf.Abs(latitude - targetCoordinates.x) * 100000; // 0.0002 * 100000 = 2
        float y1 = Mathf.Abs(longtitude - targetCoordinates.y) * 100000; // 0.0011 * 100000 = 110
